...
*/
  ...
  async function fetchData() {
    ...
    const combinedDataSet = Object.keys(resultsMappedById_F).reduce(
      (acc, key) => {
  ...
  async function fetchDataFromAPI_F() {
    ...
    const response = await fetch(
      "/favourites/getThisUsersFavourites/" + user_email,
      {
        method: "GET",
      }
    );
  ...
  function getMyFavourites() {
    const append_image = (props) => {
      return props ? require("../images/" + props) : "";
    };
    async function removeFavourites(property_id, user_email) {
      const response = await fetch(
        "/favourites/getRaingID/" + property_id + "/" + user_email,
        {
          method: "GET",
        }
      );
      ...
      axios
        .delete("/favourites/deleteRating/" + f_id)
        .then((res) => {
                  ...
                  ({
                    ...
                    <div class="col-sm-3 col-md-3 pb-2">
                      <div class="card card-outline-info border-0">
                        ...
                        <button
                          onClick={() => removeFavourites(_id, user_email)}
                          class="btn btn-primary btn-primary-f "
                          type="button"
  ...
  return (
    <button
      onClick={() => getMyFavourites()}
      class="btn btn-primary btn-primary-f "
      type="button"
      id="dropdownMenuButton"
      aria-expanded="false"
      style={{ fontSize: 18, background: "white", color: "red" }}
    >
      {" "}
      <FaRegHeart />{" "}
    </button>
